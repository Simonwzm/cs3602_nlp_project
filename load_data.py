...
@cache_results(_cache_fp='slu_cache', _refresh=True)
def load_slu(path,unigram_embedding_path=None,bigram_embedding_path=None,index_token=True,
                   normlize={'char':True,'bigram':True,'word':False}):
    # from fastNLP.io.loader import ConllLoader
    ...
    for k,v in datasets.items():
        # ignore the word segmentation tag
        v.apply_field(lambda x: list(x),'utt','chars')
        v.apply_field(get_bigrams,'chars','bigrams')
        v.apply_field(lambda x: x, 'tags', 'target')
