...
from slu.dca_net.model.torch_crf import CRF
from slu.dca_net.layers.dynamic_rnn import DynamicLSTM
from slu.dca_net.data_util import config


class Joint_model(nn.Module):
    def __init__(self, n_class, n_tag, vocab_size, embed_size, batch_size):
        super(Joint_model, self).__init__()
        hidden_dim, max_length = config.hidden_dim, config.max_len
        ...
        config.emb_dim = config.embed_size
    ...
    def forward(self, batch):
        # x, x_char = x
        tag_ids = batch.tag_ids  # ! batch_size x num_lengths
        tag_mask = batch.tag_mask  # ! batch_size x num_lengths
        input_ids = batch.input_ids  # ! batch_size x num_lengths
        intent_ids = batch.intent_ids  # ! batch_size X 1
        # lengths = batch.lengths

        x_len = torch.sum(input_ids != 0, dim=-1)
        # x_emb = self.emb_drop(self.embed(x))
        #! x_emb: batch_size x num_lengths x d_model
        x_emb = self.embed(input_ids)  # ! packed input_ids can have trailing zeros.

        #! H/H_I/H_S: 32 x 32 x 128
        H, (_, _) = self.biLSTM(x_emb, x_len)
        H_I, H_S = self.I_S_Emb(H, H, tag_mask)
        H_I, H_S = self.T_block1(H_I + H, H_S + H, tag_mask)
        H_I_1, H_S_1 = self.I_S_Emb(H_I, H_S, tag_mask)
        H_I, H_S = self.T_block2(H_I + H_I_1, H_S + H_S_1, tag_mask)
    ...
    def loss1(self, logits_intent, logits_slot, batch):
        # ! logits_intent: batch_size x num_slots, logits_slot: batch_size x num_lengths x num_slots

        tag_ids = batch.tag_ids  # ! batch_size x padded_lengths
        tag_mask = batch.tag_mask  # ! batch_size x padded_lengths
        intent_label = batch.intent_ids
        slot_label = tag_ids

        mask = tag_mask[:, 0:logits_slot.size(1)]
    ...
    def decode(self, label_vocab, batch):
        ...
        labels = batch.labels
        logits_intent, logits_slot = self.forward(batch)
        pred_intent, pred_slot = self.pred_intent_slot(logits_intent, logits_slot, batch.tag_mask)
        loss_intent, loss_slot = self.loss1(logits_intent, logits_slot, batch)
        # prob = output[0]
        predictions = []
        for i in range(batch_size):
            # pred = torch.argmax(prob[i], dim=-1).cpu().tolist()
            pred = torch.cat([x.unsqueeze(-1).float() for x in pred_slot[i]], dim=-1)
...
class I_S_SelfAttention(nn.Module):
    ...
    def forward(self, intent, slot, mask):
        #! intent: 32 x 32 x 128
        #! slot: 32  x 32 x 128
        #! mask: 32 x 26
        extended_attention_mask = mask.unsqueeze(1).unsqueeze(2)  # ! batch_size x 1 x 1 x num_length
